...
export default function ApplyInternshipModal({ show, setShow, internship }) {
  ...
  const handleSubmit = (e) => {
    ...
    //   save internship 
    const existing = JSON.parse(localStorage.getItem("myInternships")) || [];
    const isDuplicate = existing.some(
      (item) => item.title === internship.title && item.company === internship.company
    );

    if (!isDuplicate) {
      const updated = [...existing, internship];
      localStorage.setItem("myInternships", JSON.stringify(updated));
    }
  ...
  return (
    <>
      ...
      <SuccessModal
        ...
        message={`You have successfully applied for the ${internship.title} position at ${internship.company}.`}
        buttonText="View My Internships"
        onButtonClick={handleViewInternships}
      />
    </>
  );
}
